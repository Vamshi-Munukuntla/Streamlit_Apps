...
lottie_airplane = load_lottie_url('https://assets4.lottiefiles.com/packages/lf20_jhu1lqdz.json')
st_lottie(lottie_airplane, speed=1, height=200, key='initial', )

st.title("Major US Airline Job application")
st.write('by Vamshi Munukuntla')

password_attempt = st.text_input('Please Enter the Password')
if password_attempt != 'Vamshi@takehome_assignment':
...
"""
From some quick googling, I found that the haversine distance is a good 
approximation for distance. At least good enough to get distance between 
airports! 

Haversine distance can be off by up to 0.5%, because the earth is not actually 
a sphere. It looks like the latitudes and longitudes are in degrees, so I'll
make sure to have a way to account for that as well.

The haversine distance formula is labeled below, followed by an implementation 
in python.
"""
...
"""
Now, we need to test out our function! The distance between the default 
points is 18,986 kilometers, but feel free to try out your own points of interest.
"""

long1 = st.number_input('Longitude 1', value=2.55)
long2 = st.number_input('Latitude 2', value=172.00)
...
""" 
We have the Haversine distance implemented, and we also have proven to 
ourselves that it works reasonably well. 
Our next step is to implement this in function!
"""
...
"""
To use this function, select an airport from the airports provided 
in the dataframe and this application will find the distance between each one,
and return a list of the airports closest to furthest.
"""
...
"""
How would you transform a collection of searches into a numeric vector
representing a trip?
"""
...
In order to do this, I would group by the origin, the destination, and the 
departure data (for the departure date, often customers will change around
this departure data, so we should group by the date plus or minus at least 1 
buffer day to capture all the correct dates).

Additionally, we can see that often users search from an entire city, and then 
shrink that down into a specific airport. So we should also consider a group of 
individual queries from cities and airports in the same city, as the same
search, and do the same for destination.

From that point, we should add these important columns to each unique search.
"""

example_df = pd.DataFrame(columns=['user_id', 'number_of_queries',
                                   'round_trip', 'distance',
                                   'number_unique_destinations',
                                   'number_unique_origins', 'datetime_first_searched',
                                   'average_length_of_stay', 'length_of_search'])

example_row = {'user_id': 98953, 'number_of_queries': 5, 'round_trip': 1,
               'distance': 893, 'number_unique_destinations': 1,
...
There are two solid options for comparing two 
entirely numeric rows, 

- the euclidian distance (which is just the straight line 
difference between two values), and 

- the manhattan distance (think of this as the 
